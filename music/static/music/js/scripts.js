$(function() {
	...
	$('#searchForm').on('submit', function(event){
      ...
      $('.form form button').addClass('disabled');
	    search_music();
	});
    ...
    $.ajax({
	    ...
	    success : function(json) {
          $('.form form button').addClass('disabled');
	        ...
	        ajax_result(json);
	        play_music();
          $('.form form button').removeClass('disabled');
	    },
	    // handle a non-successful response
	    error : function(xhr,errmsg,err) {
	        $('#results').html("<div class='alert-box alert radius' data-alert>Oops! We have encountered an error: "+errmsg+
...
function popular_result(json)  {
    ...
    for (var i=0;i<elements.length;i++){
      ...
      $('#result ol').append('<li style="position: relative;"><a href="#" data-src="'+url+'" data-artist="'+artist+'" data-title="'+title+'">'+artist+'</a> — '+title+'</li>');
    }
}
...
function search_cover(artist, title) {
    ...
    $('.cover_block').removeClass('fadeInLeft').addClass('fadeOutLeft');
    $('.cover_block').html('');
    ...
    $.getJSON(fmurl, function(data) {
        $.each(data, function(i, item){
            ...
            if(item.album && item.album.image[3]['#text'] !=''){
                ...
                $('.cover_block').removeClass('fadeOutLeft').addClass('fadeInLeft');
...
function playNow(artist, title)  {
  $('.playNow').html('');

  $('.playNow').append('<p><span>Playing Now:</span><br>'+artist+' — '+title+'</p>')
}